...
using Bets.Bets.Repository;
using Bets.Bets.DTO;
using Bets.Bets.Services;


namespace Bets.Bets.Controllers;
...
public class BetController : Controller
{
    ...
    public async Task<IActionResult> Post([FromBody] BetDTORequest request)
    {
        try
        {
            ...
            try
            {
                var updatedBets = await _oddService.UpdateOdd(request.MatchId, request.TeamId, request.BetValue);
            }
            catch (Exception ex)
            {
